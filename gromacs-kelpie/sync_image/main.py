...
KELPIE_MANAGED_UPLOAD_FOLDER = "kelpie_managed_upload_folder"
if os.path.exists(KELPIE_MANAGED_UPLOAD_FOLDER):
    shutil.rmtree(KELPIE_MANAGED_UPLOAD_FOLDER)
os.makedirs(KELPIE_MANAGED_UPLOAD_FOLDER)
...
def main():
    ...
    if len(result) <= 1:
        ...
        os._exit(1) # The job will be retired by other nodes for 3 times - Kelpie
    else:
        print(result)  
    ...
    if cpt_exists:
        ...
        if steps_done is None: 
            ...
            os._exit(0) # The job is completed - Kelpie
        else: #
    ...
    while True:  
        ...
        # Kelpie Upload: new files - trr, edr, log, xtc(optional), gro(final)
        for file_path in new_files:
            tempFile = os.path.join(KELPIE_MANAGED_UPLOAD_FOLDER, os.path.basename(file_path))
            shutil.copy(file_path, tempFile) # Make a copy

        # Kelpie Upload: latest checkpoint and previous checkpoint if present
        for local_path, file_name in [ (CPT_LOCAL,      CPT_FILE), 
                                       (CPT_PREV_LOCAL, CPT_PREV_FILE),
                                       (HISTORY_LOCAL,  HISTORY_FILE) ]:
            if not os.path.exists(local_path):
                continue
            temp_file = os.path.join(KELPIE_MANAGED_UPLOAD_FOLDER, file_name)
    ...
    print( "The runtime was {:.3f} seconds".format(g_end - g_start) ) # completed or failed 

    # The existing uploads will be finished by Kelpie, 
    if task_completed == True:
        os._exit(0) # The job is completed - Kelpie
    else:
        os._exit(1) # The job will be retired by other nodes for 3 times - Kelpie   
